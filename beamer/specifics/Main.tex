...
\begin{frame} % -------------------------------------------------------- FRAME %
    ...
    \begin{tikzpicture}[
        ...
        \node (bibtex)
            [yshift=-\yOffs,signal,shape = signal,signal from =west,signal to=east,below of=aux1]
            {bibtex};
        ...
        \node (latex3)
            %[xshift=\xOffs,yshift=-\yOffs,rotate=-45,signal,shape = signal,signal from =west,signal to=east,below right of=aux2]
        ...
        \node[inputfile] (bib)
            [xshift=-2* \xOffs,left of=bibtex]
            {\code{.bib}};

        \node[inputfile] (bst)
            [yshift=-\yOffs,below of=bibtex]
            {\code{.bst}};
        ...
        \draw[thick,-{stealth}]
            (bib.east) -- (bibtex.west);

        \draw[thick,-{stealth}]
            (bst.north) -- (bibtex.south);

        \draw[thick,-{stealth}]
            (bibtex.east) -- (bbl.west);
    ...
    \source{\emph{The \LaTeX\ Companion}, 2nd Ed., p.688, Fig. 12.1}
\end{frame}
