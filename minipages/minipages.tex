...
\noindent\begin{minipage}[t][][t]{0.33\textwidth}
    Alternatively to inserting  a negative \texttt{vshift}, we can  also use a
    \texttt{\textbackslash{}raisebox}  command to  raise  the entire  minipage
    upwards.
...
\end{minipage}}
    ...
    If you  overlay this page  and the previous  one, they should  be visually
    indistinguishable. So which approach you prefer is up to you. The usage of
    the \texttt{\textbackslash{}raisebox}  command is the cleaner  approach in
    the author's opinion, but whether the reader holds the author's opinion in
...
\fbox{\begin{minipage}[t][][b]{0.1\textwidth}
    And let's make another one right next to it.
    \end{minipage}}
...
\fbox{\begin{minipage}[t][][t]{0.1\textwidth}
    Whereas this one has been positioned via \texttt{[t][][t]}.
    \end{minipage}}
